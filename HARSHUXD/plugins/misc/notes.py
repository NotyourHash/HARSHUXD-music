...
from HARSHUXD.utils.daxx_ban import admin_filter
from HARSHUXD.mongo.notesdb import *
from HARSHUXD.utils.notes_func import GetNoteMessage, exceNoteMessageSender, privateNote_and_admin_checker
from HARSHUXD.utils.yumidb import user_admin
...
async def PrivateNote(client, message):
    ...
    if len(message.command) >= 2:
        ...
        if await is_pnote_on(chat_id):
            await message.reply(
                "Your notes are currently being sent in private. HARSHUXD will send a small note with a button which redirects to a private chat.",
                quote=True
            )
        else:
            await message.reply(
